...
Реализуй метод ourInterruptMethod таким образом, чтобы он прерывал
нить TestThread. Исправь остальной код программы, если это необходимо.
...
2. Метод run должен выводить надпись "he-he" каждые пол секунды, пока не будет вызван метод
ourInterruptMethod.
        ...
        public void run() {
            while (!isStopped) {
                try {
                   ...
                   // System.out.println("Поток преван ");
                }
            }
                System.err.println("Поток завершился");
